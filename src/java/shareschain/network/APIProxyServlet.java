...
public final class APIProxyServlet extends AsyncMiddleManServlet {
            ...
            if (APIProxy.isActivated() && isForwardable(requestType)) {
                ...
                if (!initRemoteRequest(request, requestType)) {
                    if (Nodes.getNodes(node -> node.getState() == Node.State.CONNECTED, 1).size() >= 1) {
                        responseJson = JSONResponses.API_PROXY_NO_OPEN_API_NODES;
                    } else {
                        responseJson = JSONResponses.API_PROXY_NO_PUBLIC_NODES;
                    }
                } else {
    ...
    private boolean initRemoteRequest(HttpServletRequest clientRequest, String requestType) {
        ...
        if (!APIProxy.forcedServerURL.isEmpty()) {
            ...
            Node servingNode = APIProxy.getInstance().getServingNode(requestType);
            if (servingNode == null) {
                return false;
            }
            uri = servingNode.getNodeApiUri();
            clientRequest.setAttribute(REMOTE_SERVER_IDLE_TIMEOUT, servingNode.getApiServerIdleTimeout());
        }
        uri.append("/shareschain");
