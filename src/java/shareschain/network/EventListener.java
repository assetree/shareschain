...
import shareschain.blockchain.TransactionProcessor;
import shareschain.database.TransactionalDB;
...
/**
 * EventListener listens for node, block, transaction and account ledger events as
 ...
 * has not been received within shareschain.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by shareschain.apiMaxEventUsers and
 * the maximum number of registrations per user is specified by shareschain.apiMaxUserEventRegistrations.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDB.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Shareschain.getIntProperty("shareschain.apiMaxEventUsers", 32);

    /** Maximum user registrations */
    static final int maxUserEventRegistrations = Shareschain.getIntProperty("shareschain.apiMaxUserEventRegistrations", 2);

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Shareschain.getIntProperty("shareschain.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Shareschain.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Shareschain.getTransactionProcessor();
    ...
    /** Node events - update API comments for EventRegister and EventWait if changed */
    static final List<Nodes.Event> nodeEvents = new ArrayList<>();
    static {
        nodeEvents.add(Nodes.Event.ADD_ACTIVE_NODE);
        nodeEvents.add(Nodes.Event.ADD_NODE);
        nodeEvents.add(Nodes.Event.BLACKLIST);
        nodeEvents.add(Nodes.Event.CHANGE_ACTIVE_NODE);
        nodeEvents.add(Nodes.Event.CHANGE_ANNOUNCED_ADDRESS);
        nodeEvents.add(Nodes.Event.CHANGE_SERVICES);
        nodeEvents.add(Nodes.Event.REMOVE_NODE);
        nodeEvents.add(Nodes.Event.UNBLACKLIST);
    }
    ...
    /** Shareschain event listeners */
    private final List<ShareschainEventListener> sharesChainEventListeners = new ArrayList<>();
     ...
     *
     * Shareschain event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Shareschain event registrations
     ...
     *
     * @param   eventRegistrations      Shareschain event registrations
     * @throws  EventListenerException  Invalid Shareschain event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<ShareschainEventListener> it = sharesChainEventListeners.iterator();
                while (it.hasNext()) {
                    ShareschainEventListener listener = it.next();
                ...
                if (addListener) {
                    ShareschainEventListener listener = new ShareschainEventListener(event);
                    listener.addListener();
                    sharesChainEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void removeEvents(List<EventRegistration> eventRegistrations) {
        ...
        try {
            ...
            //
            if (sharesChainEventListeners.isEmpty()) {
                deactivateListener();
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            sharesChainEventListeners.forEach(ShareschainEventListener::removeListener);
        } finally {
            lock.unlock();
        }
    ...
    /**
     * Shareschain event listener
     */
    private class ShareschainEventListener {

        /** Event handler */
        private final ShareschainEventHandler eventHandler;

        /**
         * Create the Shareschain event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public ShareschainEventListener(EventRegistration eventRegistration) throws EventListenerException {
            Enum<? extends Enum> event = eventRegistration.getEvent();
            if (event instanceof Nodes.Event) {
                eventHandler = new NodeEventHandler(eventRegistration);
        ...
        /**
         * Return the Shareschain event
         *
         * @return                  Shareschain event
         */
        ...
        /**
         * Add the Shareschain listener for this event
         */
        ...
        /**
         * Remove the Shareschain listener for this event
         */
        ...
        /**
         * Return the hash code for this Shareschain event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Shareschain events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof ShareschainEventListener) &&
                    eventHandler.equals(((ShareschainEventListener)obj).eventHandler));
        }

        /**
         * Shareschain listener event handler
         */
        private abstract class ShareschainEventHandler {
            ...
            /** Shareschain listener event */
            protected final Enum<? extends Enum> event;

            /**
             * Create the Shareschain event handler
             *
             * @param   eventRegistration   Event registration
             */
            public ShareschainEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof ShareschainEventHandler) &&
                                        owner == ((ShareschainEventHandler)obj).owner &&
                                        accountId == ((ShareschainEventHandler)obj).accountId &&
                                        event == ((ShareschainEventHandler)obj).event);
            }
        }
         ...
         * Node event handler
         */
        private class NodeEventHandler extends ShareschainEventHandler implements Listener<Node> {

            /**
             * Create the node event handler
             *
             * @param   eventRegistration   Event registration
             */
            public NodeEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public void addListener() {
                Nodes.addListener(this, (Nodes.Event)event);
            }
            ...
            @Override
            public void removeListener() {
                Nodes.removeListener(this, (Nodes.Event)event);
            }
            ...
            public void notify(Node node) {
                dispatch(new PendingEvent("Node." + event.name(), node.getHost()));
            }
         ...
         */
        private class BlockEventHandler extends ShareschainEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends ShareschainEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends ShareschainEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Shareschain listener event
        ...
        /**
         * Return the Shareschain listener event
         *
         * @return                  Shareschain listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
