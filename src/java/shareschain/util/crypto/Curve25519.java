...
final class Curve25519 {
     ...
     *   k [out] your private key for key agreement  秘钥协议
     *   k  [in]  32 random bytes 32位随机数
     */
    public static void clamp(byte[] k) {
        k[31] &= 0x7F;//十进制127，二进制1111111
        k[31] |= 0x40;//十进制64，二进制1000000
        k[ 0] &= 0xF8;//十进制248，二进制11111000
    }
    ...
    public static boolean isCanonicalSignature(byte[] v) {
        ...
        //System.out.println("vCopy ===== "+Arrays.toString(vCopy));
        byte[] tmp = new byte[32];
        divmod(tmp, vCopy, 32, ORDER, 32);
        //System.out.println("tmp ===== "+ Arrays.toString(tmp));
        //System.out.println("vCopy divmod end ===== "+Arrays.toString(vCopy));
        for (int i = 0; i < 32; i++){
            if (v[i] != vCopy[i])
                return false;
        }
    ...
    private static void core(byte[] Px, byte[] s, byte[] k, byte[] Gx) {
        //new long10() 初始化一个Long对象，共有10个属性赋值位0
        long10
                dx=new long10(),
                t1=new long10(),
                t2=new long10(),
