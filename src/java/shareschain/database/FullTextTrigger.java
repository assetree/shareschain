 ...
 *   CREATE ALIAS FTL_CREATE_INDEX FOR "shareschain.database.FullTextTrigger.createIndex"
 *       CALL FTL_CREATE(schema, table, columnList)
 *   CREATE ALIAS FTL_DROP_INDEX FOR "shareschain.database.FullTextTrigger.dropIndex"
 *       CALL FTL_DROP(schema, table)
 *   CREATE ALIAS FTL_SEARCH FOR "shareschain.database.FullTextTrigger.search"
 *       CALL FTL_SEARCH(schema, table, query, limit, offset)
 *
 * FTL_CREATE_INDEX is called to create a fulltext index for a table.  It is
 * provided as a convenience for use in ShareschainDBVersion when creating a new index
 * after the database has been created.
 *
 * FTL_DROP_INDEX is called to drop a fulltext index for a table.  It is
 * provided as a convenience for use in ShareschainDBVersion when dropping a table
 * after the database has been created.
 *
 ...
 *   CREATE TRIGGER trigger_name AFTER INSERT,UPDATE,DELETE ON table_name FOR EACH ROW CALL "shareschain.database.FullTextTrigger"
 */
public class FullTextTrigger implements Trigger, TransactionalDB.TransactionCallback {
     ...
     *
     * This method should be called from ShareschainDBVersion when performing the database version update
    ...
    public static void migrateToV7(BasicDB db) {
        ...
        try (Connection conn = db.getConnection("PUBLIC");
            ...
            //Recreate the triggers since their initialization failed when executed with Lucene V5 index, thus indexTriggers
            // is empty
            try (ResultSet rs = qstmt.executeQuery("SELECT * FROM FTL.INDEXES")) {
                while (rs.next()) {
                    String schema = rs.getString("SCHEMA");
            ...
            //Now having the triggers initialized, rebuild the index
            reindex(conn);

        } catch (SQLException e) {
            Logger.logErrorMessage("Unable to migrate NRS fulltext search support", e);
    ...
    private static void getIndexAccess(Connection conn) throws SQLException {
        ...
        try {
            ...
            if (exc instanceof IndexFormatTooOldException) {
                Logger.logInfoMessage("Lucene index needs rebuild. Should be done automatically.", exc);
            } else {
